...
class GetProducts extends Command
{
    ...
    {
        try {
            $output->writeln("<info>Starting Song Import...</info>");

            $outputStyle = new OutputFormatterStyle('blue', 'black', ['bold', 'blink']);
            $output->getFormatter()->setStyle('updated', $outputStyle);

            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $categoryObj = $objectManager->get('\Magento\Catalog\Model\CategoryFactory')->create()->loadByAttribute('name',
                'Artists');
            ...
            $__count_shows = 0;
            $__count_songs = 0;
$_artist_skip_amount = 5;
$_show_skip_amount = 0;
$_show_limit_amount = 10000;
            foreach ($subcategories as $subcategorie) {
                $output->writeln('<info>     artist:' . $subcategorie->getName() . ' magento.categoryid:' . $subcategorie->getId() . ' ' . number_format(memory_get_peak_usage()) . ' bytes</info>');
                //if($subcategorie->getName()=='STS9'){
                //if($subcategorie->getName()=='Of a Revolution'){
                //if($subcategorie->getName()=='Railroad Earth'){
                $_hold_name_hack = $subcategorie->getName();
                ...
                if($__count_cats > $_artist_skip_amount) {
                    $_subcategory_info = [];
                    ...
                    //$output->writeln('<info>     On Artist #' . $__count_cats . ' ' . $_subcategory_info['name'] . ' Reading RSS --> ' . $_the_rss_url . '</info>');
                    $output->writeln('<info>          Reading RSS --> ' . $_the_rss_url . '</info>');
                    $output->writeln('          skippng the first ' . $_show_skip_amount . ' show(s)...');
                    ...
                    if ($__count_cats >= 0) {
                        ...
                        foreach ($collection_rss->channel->item as $_item) {
                            $__count_show_loop = $__count_show_loop +1;
                            //if ($_sub_item_count_per_artist >= 544) {
                            //if (($__count_cats != 1) && ($_sub_item_count_per_artist != 278)) {
                            if (($__count_shows >= $_show_skip_amount) && ($__count_show_loop <= $_show_limit_amount)) {
                                //sleep(5);
                                //echo 'ssssa';exit;
                                ...
                                $_show_vars_feed['feed'] = file_get_contents(str_replace('/details/', '/metadata/',
                                    $_show_vars['guid']), false, $context);
                                ...
                                //$output->writeln('<info>     artist:'.sprintf('%03d',$__count_cats).'-of-'.sprintf('%03d',count($subcategories)).' | '.$_subcategory_info['name'].'</info>');
                                //$output->writeln('<info>            show:'.sprintf('%03d',$__count_show_loop).'-of-'.sprintf('%03d',$_how_many_feeds).' | '.$_show_vars['title'].'</info>');
                                $output->write('<info>          artist:' . $_hold_name_hack . ' ' . sprintf('%03d', $__count_cats) . '-of-' . sprintf('%03d', count($subcategories)) . ' | ' . $_subcategory_info['name'] . ' show:' . sprintf('%03d', $__count_show_loop) . '-of-' . sprintf('%03d', $_how_many_feeds) . ' | ' . $_show_vars['title'] . '</info>');
                                //$output->writeln('<info>            year:'.$_show_vars['year'].'</info>');
                                //$output->writeln('<info>            venue:'.$_show_vars['venue'].'</info>');
                                //$output->writeln('<info>            tape:'.$_show_vars['taper'].'</info>');
                                //$output->writeln('<info>            transferer:'.$_show_vars['transferer'].'</info>');
                                //$output->writeln('<info>            feed:'.$_show_vars_feed['feed_url'].'</info>');
                                //$output->writeln('<info>            memory:'.number_format(memory_get_peak_usage()).' bytes</info>');
                                //$output->write('                 ');
                                $output->write('     ');
                                foreach ($_show_vars_feed['json']->files as $_song) {
                                    ...
                                    if ($this->endsWith($_song->name, '.flac') && isset($_song->title)) {
                                        ...
                                        //						}
                                        if ($_update_flag == 1) {
                                            $output->write('<updated>U</updated>');
                                            //$output->writeln('<comment>artist:'.sprintf('%03d',$__count_cats).'-of-'.sprintf('%03d',count($subcategories)).':show:'.sprintf('%03d',$__count_show_loop).'-of-'.sprintf('%03d',$_how_many_feeds).':song:#'.sprintf('%04d',$__count_songs).':updated:</comment>');

                                            //$output->writeln('<comment>artist:'.sprintf('%03d',$__count_cats).'-of-'.sprintf('%03d',count($subcategories)).'</comment>');
                                            //$output->writeln('<comment>       show:'.sprintf('%03d',$__count_show_loop).'-of-'.sprintf('%03d',$_how_many_feeds).'</comment>');
                                            //$output->writeln('<comment>            song:#'.sprintf('%04d',$__count_songs).'</comment>');
                                            //$output->writeln('<comment>                 updated:</comment>');
                                            //$output->writeln('<comment>                         sku:'.$_song_var['sku'].'</comment>');
                                            //$output->writeln('<comment>                         title:'.$_song_var['title'].'</comment>');
                                            //$output->writeln('<comment>                         name:'.$_song_var['name'].'</comment>');
                                            //$output->writeln('<comment>                         url_key:'.$product->getUrlKey().'</comment>');
                                            //$output->writeln('<comment>                         url:'.$_song_var['prod_url'].'</comment>');
                                            //$output->writeln('<comment>                         source:'.$_song->source.'</comment>');
                                        } else {
                                            $output->write('<info>A</info>');
                                            //$output->writeln('<info>artist:'.sprintf('%03d',$__count_cats).'-of-'.sprintf('%03d',count($subcategories)).':show:'.sprintf('%03d',$__count_show_loop).'-of-'.sprintf('%03d',$_how_many_feeds).':song:#'.sprintf('%04d',$__count_songs).':added:</info>');

                                            //$output->writeln('<info>artist:'.sprintf('%03d',$__count_cats).'-of-'.sprintf('%03d',count($subcategories)).'</info>');
                                            //$output->writeln('<info>       show:'.sprintf('%03d',$__count_show_loop).'-of-'.sprintf('%03d',$_how_many_feeds).'</info>');
                                            //$output->writeln('<info>            song:#'.sprintf('%04d',$__count_songs).'</info>');
                                            //$output->writeln('<info>                 added:</info>');
                                            //$output->writeln('<info>                       sku:'.$_song_var['sku'].'</info>');
                                            //$output->writeln('<info>                       title:'.$_song_var['title'].'</info>');
                                            //$output->writeln('<info>                       name:'.$_song_var['name'].'</info>');
                                            //$output->writeln('<info>                       url_key:'.$product->getUrlKey().'</info>');
                                            //$output->writeln('<info>                       url:'.$_song_var['prod_url'].'</info>');
                                            //$output->writeln('<info>                       source:'.$_song->source.'</info>');
                                        }
                    ...
                    $output->writeln('          skippng the first ' . $_artist_skip_amount . ' artist(s)...');
                }
            }
    ...
    public function createOrGetId($attributeCode, $label)
    {
        try {
            ...
            $optionId = $this->getOptionId($attributeCode, strtolower(trim($label)));
            if (!$optionId) {
                // If no, add it.
                ...
                $option->setLabel(strtolower(trim($label)));
