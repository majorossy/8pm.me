...
### ProductRefresher
Updates existing products with fresh Archive.org data using efficient batch API.
...
**Available Fields:**
- `rating` - Average rating (batch API)
- `reviews` - Number of reviews (batch API)
- `downloads` - Total download count (batch API)
- `trending` - Weekly/monthly downloads (batch API)
- `length` - Track duration (per-show API, slow)

```php
// Refresh with default fields (fast batch API)
$result = $productRefresher->refresh(
    artistName: 'STS9',
    fields: ['rating', 'reviews', 'downloads', 'trending'],  // default
    force: false,
    limit: null,
    progressCallback: fn($total, $current, $msg) => $output->writeln($msg)
);
// Returns: ['products_updated' => 150, 'api_calls' => 6, 'cache_hits' => 0, ...]

// Include length field (requires slow per-show API)
$result = $productRefresher->refresh('STS9', ['rating', 'length'], force: true);

// Dry run to preview API calls
$preview = $productRefresher->dryRun('STS9');
// Returns: ['estimated_api_calls' => 6, 'batch_api_calls' => 6, 'per_show_api_calls' => 0, ...]
```
...
Updates existing products with fresh data from Archive.org using efficient batch API.

**Performance:** For 523 shows, makes ~6 API calls instead of 523 individual calls.

```bash
# Dry run - preview API calls needed
bin/magento archive:refresh:products "STS9" --dry-run

# Refresh with default fields (fast: rating, reviews, downloads, trending)
bin/magento archive:refresh:products "STS9"

# Refresh with specific fields only
bin/magento archive:refresh:products "Grateful Dead" --fields=rating,reviews

# Include all fields including slow 'length' (requires per-show API)
...
bin/magento archive:refresh:products "STS9" --limit=10

# Force refresh (bypass 1-week cache)
bin/magento archive:refresh:products "STS9" --force
```
...
- **Purpose**: Updates stats, not product content; does not create new products
- **Performance**: Batch API fetches 100 shows per call
...
| `archive_downloads` | int | Archive.org total download count |
| `archive_downloads_week` | int | Archive.org downloads this week |
| `archive_downloads_month` | int | Archive.org downloads this month |
