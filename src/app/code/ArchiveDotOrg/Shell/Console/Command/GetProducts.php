...
use ArchiveDotOrg\Shell\Helper;
...
//if($subcategorie->getId() ==2641){ // Todd Sheaffer
                    ...
                    if ($__count_cats >= 0) {
                        ...
                        foreach ($collection_rss->channel->item as $_item) {
                            ...
                            if ($_sub_item_count_per_artist >= 1345) {
                                //sleep(5);
                                //echo 'ssssa';exit;
                                ...
                                foreach ($_show_vars_feed['json']->files as $_song) {
                                    ...
                                    if ($this->endsWith($_song->name, '.flac') && isset($_song->title)) {
                                        ...
                                        $atr_archive_collection = $this->createOrGetId('archive_collection',
                                        ...
                                        $product->setUrlKey(strtolower(preg_replace('#[^0-9a-z]+#i', '-',
                                            rtrim(mb_strimwidth($_song_var['sku'], 0, 64, '', 'UTF-8')))));
                                        $product->setIdentifier($_show_vars['identifier']);
                                        $product->setDir($_show_vars['dir']);
                                        $product->setSongUrl($_show_vars['server_one'] . '/' . $_show_vars['dir'] . '/' . rtrim(mb_strimwidth(substr($_song->name,
                                                0, -5), 0, 64, '', 'UTF-8')) . '.flac');
                                        //
